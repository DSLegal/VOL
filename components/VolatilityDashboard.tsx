...
export default function VolatilityDashboard() {
  ...
  return <div className="app-shell">
    <header className="topbar">
      <button className="brand" onClick={() => setView("lab")}><span className="brand-mark">V</span><span><strong>STOP / ROOM</strong><small>NQ history · MNQ sizing</small></span></button>
    ...
    <main>
      {view === "lab" && <section className="page lab-page">
        <div className="page-head">
          <div><p className="eyebrow">Price-action stop check</p><h1>Will your stop survive<br/><em>normal price movement?</em></h1><p className="lead">Mark the price that proves your trade idea wrong. This tool then shows whether that distance was tight, ordinary or wide compared with similar historical NQ movement.</p></div>
        ...
        <div className="price-action-flow">
          <article><span>1</span><div><strong>Define invalidation</strong><p>Where is the trade idea objectively wrong?</p></div><label><input aria-label="Price-action invalidation distance" type="number" min="0.25" step="0.25" value={structuralStop} onChange={event => setStructuralStop(Math.max(.25, Number(event.target.value)))}/><b>NQ points</b></label></article>
          <article><span>2</span><div><strong>Match the context</strong><p>Choose the session, trade side and first 5 or 10 minutes.</p></div></article>
        ...
        <div className={`plain-result ${stopReading.tone}`}>
        ...
        <div className="workbench">
          <aside className="control-rail">
            ...
            <div className="field"><span>First movement window <InfoMarker title="First movement window" takeaway="Use the time window that best matches how quickly your setup should respond.">This asks how far price moved against an entry during the next 5 or 10 minutes. It is not the total movement for the whole trade.</InfoMarker></span><Toggle value={String(horizon)} onChange={value => setHorizon(Number(value))} label="First movement window" options={[{ value: "5", label: "5 min" }, { value: "10", label: "10 min" }]}/></div>
            ...
            <div className="field"><span>Measurement <InfoMarker title="NQ points or relative range" takeaway="Use NQ points for stop distance. Relative range is an advanced comparison across different years.">NQ points show the actual price distance. Relative range adjusts for how large the session normally was at the time.</InfoMarker></span><Toggle value={unit} onChange={setUnit} label="Measurement" options={[{ value: "points", label: "NQ points" }, { value: "mae_atr", label: "Across years" }]}/></div>
          ...
          <div className="lab-canvas">
            ...
            <div className="quantile-grid">
              {[50, 80, 90].map((q, index) => <article className={`quantile-card q${q}`} key={q}><div><span>{index === 0 ? "Typical pullback" : index === 1 ? "Wider pullback" : "Extreme pullback"} <InfoMarker title={index === 0 ? "Typical pullback (P50)" : index === 1 ? "Wider pullback (P80)" : "Extreme pullback (P90)"} takeaway={q === 50 ? "Use this as the middle, ordinary reference—not a stop suggestion." : q === 80 ? "Two out of ten measured pullbacks were still larger." : "One out of ten measured pullbacks was still larger."}>{q === 50 ? "Half of measured pullbacks were at or below this distance, and half were larger." : q === 80 ? "Eight out of ten measured pullbacks were at or below this distance." : "Nine out of ten measured pullbacks were at or below this distance."}</InfoMarker></span><small>P{q}</small></div><strong>{fmt(valueOf(reference, q as 50 | 80 | 90), unit === "points" ? 1 : 2)}</strong><em>{unit === "points" ? "NQ points" : "× normal session range"}</em></article>)}
            </div>
            <div className="chart-panel">
              ...
              <p className="chart-caption">Each point combines the same {periodType === "month" ? "month" : "week number"} across available years. “Wider pullback” means eight out of ten measured moves against entry were at or below this distance.</p>
            </div>
          </div>
        </div>
        ...
        <section className="decision-zone">
          ...
          <div className="decision-grid">
            <article className="decision-card large"><div className="input-row"><label><span>Distance to invalidation <InfoMarker title="Price-action invalidation" takeaway="Enter the distance from entry to the price that proves your trade idea wrong.">This comes from your chart structure. The dashboard compares that distance with history but never chooses or widens it.</InfoMarker></span><div><input type="number" min="0.25" step="0.25" value={structuralStop} onChange={event => setStructuralStop(Math.max(.25, Number(event.target.value)))} /><b>points</b></div></label><label><span>Today’s normal session range <InfoMarker title="Today’s session range" takeaway="This is optional. Use it only to compare today with different years.">Enter the ATR or other consistent measure you use for the selected session. The tool translates the across-years comparison back into today’s points.</InfoMarker></span><div><input type="number" min="0.25" step="0.25" value={currentAtr} onChange={event => setCurrentAtr(Math.max(.25, Number(event.target.value)))} /><b>points</b></div></label><div className="context-callout"><span>Plain-English reading</span><strong>{stopReading.title}</strong><small>Wider pullback adjusted to today’s range: {fmt(reference.p80Atr * currentAtr, 1)} pts</small></div></div><QuantileBand row={reference} structuralStop={structuralStop}/><EvidenceNote>This compares your price-action invalidation with past pullbacks. It does not say the stop is correct or predict whether the next trade will win.</EvidenceNote></article>
...
function SeasonalityView({ data, session, setSession, horizon, setHorizon, periodType, setPeriodType, period, setPeriod }: { data: Data; session: string; setSession: (v: string) => void; horizon: number; setHorizon: (v: number) => void; periodType: PeriodType; setPeriodType: (v: PeriodType) => void; period: string; setPeriod: (v: string) => void }) {
  ...
  return <section className="page subpage">
    <PageIntro eyebrow="Seasonal patterns" title="When did pullbacks tend to need more room?" body="Compare the same month or week number across past years. Use this to prepare for changing breathing room—not to predict the next move." side={<div className="head-controls"><Select label="Session" value={session} onChange={setSession}>{SESSION_ORDER.map(item => <option key={item}>{item}</option>)}</Select><div className="field"><span>First window</span><Toggle value={String(horizon)} onChange={v => setHorizon(Number(v))} label="First movement window" options={[{ value: "5", label: "5m" }, { value: "10", label: "10m" }]}/></div></div>}/>
...
function SessionsView({ data, horizon, setHorizon }: { data: Data; horizon: number; setHorizon: (v: number) => void }) {
  ...
  return <section className="page subpage">
    <PageIntro eyebrow="Time-of-day comparison" title="A trading day does not move at one speed." body="Compare how much breathing room entries historically needed during Asia, London, the New York open, lunch and the afternoon." side={<div className="head-controls"><div className="field"><span>First window</span><Toggle value={String(horizon)} onChange={v => setHorizon(Number(v))} label="First movement window" options={[{ value: "5", label: "5m" }, { value: "10", label: "10m" }]}/></div></div>}/>
...
function RegimesView({ data, session, setSession, yearStart, setYearStart }: { data: Data; session: string; setSession: (v: string) => void; yearStart: number; setYearStart: (v: number) => void }) {
  ...
  return <section className="page subpage">
    <PageIntro eyebrow="Changing market conditions" title="Seasonality is not the whole story." body="See when the market’s normal pullback behaviour changed through time. Old history may deserve less weight when recent conditions look materially different." side={<div className="head-controls"><Select label="Session" value={session} onChange={setSession}>{SESSION_ORDER.map(item => <option key={item}>{item}</option>)}</Select></div>}/>
