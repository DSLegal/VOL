...
function InfoMarker({ title, children, takeaway }: { title: string; children: React.ReactNode; takeaway?: string }) {
  ...
  const id = useId();
  return <span className="info-marker" data-open={open}>
    <button type="button" aria-label={`Explain ${title}`} aria-expanded={open} aria-describedby={open ? id : undefined} onClick={event => { event.preventDefault(); event.stopPropagation(); setOpen(value => !value); }} onBlur={() => setOpen(false)}>i</button>
      ...
      {view === "lab" && <section className="page lab-page">
        ...
        <div className="workbench">
          <aside className="control-rail">
            ...
            <div className="field"><span>Entry horizon <InfoMarker title="Entry horizon" takeaway="Use the time window that best matches how quickly your setup should respond.">This asks how far price moved against an entry during the next 5 or 10 minutes. It is not the total movement for the whole trade.</InfoMarker></span><Toggle value={String(horizon)} onChange={value => setHorizon(Number(value))} label="Entry horizon" options={[{ value: "5", label: "5 min" }, { value: "10", label: "10 min" }]}/></div>
            <div className="field"><span>Direction lens <InfoMarker title="Direction lens" takeaway="Use Both for a general view; use Long or Short when direction matters to your setup.">Both combines long and short entries. Long measures downward movement against a long. Short measures upward movement against a short.</InfoMarker></span><Toggle value={direction} onChange={setDirection} label="Direction" options={[{ value: "pooled", label: "Both" }, { value: "long", label: "Long" }, { value: "short", label: "Short" }]}/></div>
            <div className="field"><span>Calendar lens <InfoMarker title="Month or ISO week" takeaway="Start with month; use week when you want a more detailed seasonal view.">Month pools the same calendar month across all available years. ISO week pools week numbers W01 to W53 across years.</InfoMarker></span><Toggle value={periodType} onChange={setPeriodType} label="Calendar grouping" options={[{ value: "month", label: "Month" }, { value: "week", label: "ISO week" }]}/></div>
            ...
            <div className="field"><span>Display <InfoMarker title="Points or ATR-normalised" takeaway="Use points for stop distance and ATR-normalised values for comparing different years.">Points show the actual NQ distance. ATR-normalised values show the distance relative to the usual movement of that session.</InfoMarker></span><Toggle value={unit} onChange={setUnit} label="Display unit" options={[{ value: "mae_atr", label: "ATR-normalised" }, { value: "points", label: "Points" }]}/></div>
            <button className="plain-link" onClick={() => setEvidenceOpen(true)}>How this is calculated →</button>
          </aside>

          <div className="lab-canvas">
            <div className="canvas-kicker"><div><span className={`sample-chip ${sampleTone(reference.days)}`}>{reference.days} trading days</span><InfoMarker title="Sample size" takeaway="More days usually make the comparison more dependable.">This is the number of separate eligible trading days behind the selected result. Under 20 days is thin; 20–39 needs care; 40 or more is the stronger sample band used here.</InfoMarker><span>{reference.years} years represented</span></div><span>{session} · {horizon}m · {direction}</span></div>
            <div className="quantile-grid">
              {[50, 80, 90].map((q, index) => <article className={`quantile-card q${q}`} key={q}><div><span>P{q} <InfoMarker title={`P${q} adverse excursion`} takeaway={q === 50 ? "Use this as the middle, ordinary reference—not a stop suggestion." : q === 80 ? "This is the main wider-noise comparison, but 20% of observations were larger." : "This shows tail context; 10% of observations were still larger."}>{q === 50 ? "Half of measured moves against the entry were at or below this level, and half were larger." : q === 80 ? "Eight out of ten measured moves against the entry were at or below this level. Two out of ten were larger." : "Nine out of ten measured moves against the entry were at or below this level. One out of ten was larger."}</InfoMarker></span><small>{index === 0 ? "Typical" : index === 1 ? "Wider" : "Tail"}</small></div><strong>{fmt(valueOf(reference, q as 50 | 80 | 90), unit === "points" ? 1 : 2)}</strong><em>{unit === "points" ? "NQ points" : "× session ATR"}</em></article>)}
            </div>
            <div className="chart-panel">
              <div className="panel-head"><div><p className="eyebrow">{periodType === "month" ? "Month-of-year" : "ISO week-of-year"} profile</p><h2>P80 adverse excursion <InfoMarker title="How to read this chart" takeaway="Look for broad differences, then check sample size and uncertainty before acting.">Each dot is the wider-noise P80 value for the same session and horizon. A higher dot means price historically needed more breathing room in that period.</InfoMarker></h2></div><div className="active-value"><span>{resolvedPeriod}</span><strong>{fmt(valueOf(reference, 80), unit === "points" ? 1 : 2)}</strong></div></div>
        ...
        <section className="decision-zone">
          ...
          <div className="decision-grid">
            <article className="decision-card large"><div className="input-row"><label><span>Your structural stop <InfoMarker title="Structural stop" takeaway="Enter the distance from your planned entry to the price that proves your trade idea wrong.">This must come from your chart structure and invalidation logic. The dashboard compares it with history but does not choose it.</InfoMarker></span><div><input type="number" min="0.25" step="0.25" value={structuralStop} onChange={event => setStructuralStop(Math.max(.25, Number(event.target.value)))} /><b>points</b></div></label><label><span>Current session ATR <InfoMarker title="Current session ATR" takeaway="Enter today’s normal session range if you want a current-volatility translation.">ATR is an average movement range. This input converts the historical ATR-normalised P80 into an approximate number of points for today’s volatility.</InfoMarker></span><div><input type="number" min="0.25" step="0.25" value={currentAtr} onChange={event => setCurrentAtr(Math.max(.25, Number(event.target.value)))} /><b>points</b></div></label><div className="context-callout"><span>This sits <InfoMarker title="Historical position" takeaway="This describes where your stop sits in the old distribution; it does not label it right or wrong.">The wording compares your structural distance with the historical P50, P80 and P90 bands selected above.</InfoMarker></span><strong>{contextPosition}</strong><small>Normalised P80 translated at current ATR: {fmt(reference.p80Atr * currentAtr, 1)} pts</small></div></div><QuantileBand row={reference} structuralStop={structuralStop}/><EvidenceNote>The coloured bands use historical raw points. The current-ATR translation is a separate normalised comparison. Neither is a pass/fail line; price structure, liquidity and invalidation still define the stop.</EvidenceNote></article>
            <article className="decision-card sizing"><p className="eyebrow">MNQ risk translation</p><h3>Whole-contract view <InfoMarker title="MNQ risk calculation" takeaway="This is a calculator result, not a recommended trade size.">MNQ moves $2 for every Nasdaq point per contract. The calculator adds your stated round-trip cost and uses only whole contracts.</InfoMarker></h3><div className="mini-inputs"><label><span>Risk budget <InfoMarker title="Risk budget" takeaway="Enter the maximum amount you have already decided to risk on this trade idea.">This is your own dollar limit before entering the trade. The tool does not decide an appropriate budget.</InfoMarker></span><div><b>$</b><input type="number" min="1" value={riskBudget} onChange={event => setRiskBudget(Math.max(1, Number(event.target.value)))}/></div></label><label><span>Round-trip cost <InfoMarker title="Round-trip cost" takeaway="Include fees for opening and closing one MNQ contract.">This is the estimated total commission and fee cost per contract for both sides of the trade.</InfoMarker></span><div><b>$</b><input type="number" min="0" step=".25" value={cost} onChange={event => setCost(Math.max(0, Number(event.target.value)))}/></div></label></div><div className="sizing-result"><span>Maximum whole MNQ under inputs <InfoMarker title="Maximum whole contracts" takeaway="A result of 0 means the stated stop and cost do not fit inside the stated budget.">The budget is divided by risk per contract, then rounded down because a fraction of one futures contract cannot be traded.</InfoMarker></span><strong>{wholeContracts}</strong><small>{money(usedRisk)} modelled risk · {money(riskBudget - usedRisk)} unallocated</small></div><div className="formula">$2 × {fmt(structuralStop, 2)} points + ${fmt(cost, 2)} = <strong>${fmt(riskPerContract, 2)}</strong> / MNQ</div><p className="fineprint">Mechanical arithmetic only; not a size recommendation.</p></article>
          </div>
        </section>
      </section>}
...
function SeasonalityView({ data, session, setSession, horizon, setHorizon, periodType, setPeriodType, period, setPeriod }: { data: Data; session: string; setSession: (v: string) => void; horizon: number; setHorizon: (v: number) => void; periodType: PeriodType; setPeriodType: (v: PeriodType) => void; period: string; setPeriod: (v: string) => void }) {
  ...
  return <section className="page subpage">
    ...
    <article className="story-card wide"><div className="panel-head"><div><p className="eyebrow">{session} · pooled direction</p><h2>ISO week-of-year profile <InfoMarker title="ISO week-of-year" takeaway="Treat isolated spikes cautiously, especially when the selected week has few days.">W01 is the first ISO calendar week of the year. The chart pools the same week number across available years; it is not one continuous year.</InfoMarker></h2></div><Toggle value={periodType} onChange={setPeriodType} label="Season view" options={[{ value: "month", label: "Month" }, { value: "week", label: "Week" }]}/></div><LineChart points={weeks.map(row => ({ label: row.period, value: row.p80Atr, detail: `${row.days} eligible days` }))} active={selectedWeek?.period} onPick={label => { setPeriodType("week"); setPeriod(label); }} formatValue={v => `${fmt(v, 2)}×`}/><div className="selected-strip"><div><span>Highest observed headline week <InfoMarker title="Highest observed week" takeaway="Highest in the old sample does not mean it will be highest this year.">This is the largest P80 value found in the stated headline slice. It is a description of history, not a forecast.</InfoMarker></span><strong>W40 · 2.614×</strong><small>95% CI 2.492–2.735 · 71 days</small></div><div><span>Lowest observed headline week</span><strong>W27 · 2.243×</strong><small>95% CI 2.140–2.344 · 71 days</small></div><div className={selectedWeek && selectedWeek.days < 40 ? "sample-alert" : "sample-ok"}><span>Selected {selectedWeek?.period ?? "week"}</span><strong>{selectedWeek ? `${selectedWeek.days} days` : "—"}</strong><small>{selectedWeek && selectedWeek.days < 20 ? "Thin sample: interpret cautiously" : "Sample shown before interpretation"}</small></div></div></article>
  </section>;
}
