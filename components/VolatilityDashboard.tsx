...
type Data = {
  meta: { generatedAt: string; analysisGeneratedAt: string; analysisManifestSha256: string; rawDbnSha256: string; records: number; firstTimestamp: string; lastTimestamp: string; timezone: string; bootstrapReplications: number; seed: number; verifiedFiles: number; disclaimer: string; supportedHorizons?: number[]; defaultHorizon?: number; dataSourceFallback?: Array<{ instrument: string; sourceId: string; rank: number; status: string; available: boolean; notes: string }>; fallbackComparability?: { pair: string; validationHorizonMinutes: number; commonDaysMinimum: number; commonDaysMaximum: number; p80RatioMinimum: number | null; p80RatioMaximum: number | null; status: string; us100Status: string }; activeDataSource?: { dataInstrument: string; sourceId: string; sourceRank: number; fallbackLevel: number; fallbackReason: string; reconciliationStatus: string }; tradingInstruments?: Record<TradingInstrument, { dollarsPerPoint: number; defaultCostPerSide: number }> };
  seasonal: SeasonalRow[];
  seasonalCI: Array<{ sourceId?: string; periodType: PeriodType; period: string; order: number; horizon: number; session: string; direction?: string; unit: Unit; metric: QuantileMetric; estimate: number; low: number; high: number; observations: number; days: number }>;
  sessionMetrics: Array<{ sourceId?: string; dataInstrument?: string; sourceRank?: number; horizon: number; session: string; direction: string; unit: Unit | "basis_points"; observations: number; days: number; p50: number; p80: number; p90: number }>;
...
const preferSourceRows = <T extends { sourceRank?: number }>(rows: T[], keyOf: (row: T) => string) => {
  const preferred = new Map<string, T>();
  for (const row of rows) {
    const key = keyOf(row);
    const current = preferred.get(key);
    if (!current || (row.sourceRank ?? Number.MAX_SAFE_INTEGER) < (current.sourceRank ?? Number.MAX_SAFE_INTEGER)) preferred.set(key, row);
  }
...
export default function VolatilityDashboard() {
  ...
  const preferredSeasonal = useMemo(() => preferSourceRows(data?.seasonal ?? [], row => `${row.periodType}|${row.period}|${row.session}|${row.horizon}|${row.direction}`), [data]);
  const availableHorizons = useMemo(() => data ? [...new Set(preferredSeasonal.map(row => row.horizon))].sort((a, b) => a - b) : HORIZON_OPTIONS, [data, preferredSeasonal]);
  ...
  const periods = useMemo(() => {
    ...
    preferredSeasonal.filter(row => row.periodType === periodType).forEach(row => unique.set(row.order, row.period));
    return [...unique.entries()].sort((a, b) => a[0] - b[0]).map(([, label]) => label);
  }, [data, preferredSeasonal, periodType]);

  const resolvedPeriod = periods.includes(period) ? period : periods[0] ?? period;
  const reference = useMemo(() => preferredSeasonal.find(row => row.periodType === periodType && row.period === resolvedPeriod && row.session === session && row.horizon === resolvedHorizon && row.direction === direction) ?? null, [preferredSeasonal, periodType, resolvedPeriod, session, resolvedHorizon, direction]);
  const series = useMemo(() => preferredSeasonal.filter(row => row.periodType === periodType && row.session === session && row.horizon === resolvedHorizon && row.direction === direction).sort((a, b) => a.order - b.order), [preferredSeasonal, periodType, session, resolvedHorizon, direction]);
  const valueOf = (row: SeasonalRow, q: 50 | 80 | 90) => unit === "mae_atr" ? row[`p${q}Atr`] : row[`p${q}Points`];
  const ciFor = (metric: QuantileMetric) => data?.seasonalCI.find(row => row.periodType === periodType && row.period === resolvedPeriod && row.session === session && row.horizon === resolvedHorizon && (row.direction ?? "pooled") === direction && row.unit === unit && (row.metric === metric || (!row.metric && metric === "p80"))) ?? null;
  ...
  return <div className="app-shell">
    <header className="topbar">
      ...
      <nav aria-label="Dashboard sections">{NAV.map(item => <button key={item.id} className={view === item.id ? "active" : ""} aria-current={view === item.id ? "page" : undefined} onClick={() => setView(item.id)}><span className="nav-full">{item.label}</span><span className="nav-short">{item.short}</span></button>)}</nav>
...
function ActiveSourceStatus({ data }: { data: Data }) {
  const source = data.meta.activeDataSource;
  return <p className="source-status"><strong>Active volatility source:</strong> {source ? `${source.dataInstrument} · ${source.sourceId}. ${source.reconciliationStatus}. ${source.fallbackReason}.` : "NQ long-history primary source."}</p>;
}
...
function SeasonalityView({ data, session, setSession, horizon, setHorizon, periodType, setPeriodType, period, setPeriod }: { data: Data; session: string; setSession: (v: string) => void; horizon: number; setHorizon: (v: number) => void; periodType: PeriodType; setPeriodType: (v: PeriodType) => void; period: string; setPeriod: (v: string) => void }) {
  const preferred = preferSourceRows(data.seasonal, row => `${row.periodType}|${row.period}|${row.session}|${row.horizon}|${row.direction}`);
  const monthly = preferred.filter(row => row.periodType === "month" && row.horizon === horizon && row.direction === "pooled");
  ...
  const weeks = preferred.filter(row => row.periodType === "week" && row.horizon === horizon && row.direction === "pooled" && row.session === session).sort((a, b) => a.order - b.order);
  const selectedWeek = weeks.find(row => row.period === period) ?? weeks[0];
  const widestWeek = weeks.reduce<SeasonalRow | null>((best, row) => !best || row.p80Atr > best.p80Atr ? row : best, null);
  const narrowestWeek = weeks.reduce<SeasonalRow | null>((best, row) => !best || row.p80Atr < best.p80Atr ? row : best, null);
  const weekCi = (row: SeasonalRow | null) => row ? data.seasonalCI.find(item => item.periodType === "week" && item.period === row.period && item.session === session && item.horizon === horizon && (item.direction ?? "pooled") === "pooled" && item.unit === "mae_atr" && item.metric === "p80") : null;
  const widestCi = weekCi(widestWeek);
  const narrowestCi = weekCi(narrowestWeek);
...
function EvidenceView({ data }: { data: Data }) {
  ...
  return <section className="page subpage evidence-page">
    ...
    <div className="evidence-layout"><aside><p className="eyebrow">Classification</p>{classes.map(item => <button key={item} className={filter === item ? "active" : ""} aria-pressed={filter === item} onClick={() => setFilter(item)}>{item === "all" ? "All claims" : item}<span>{item === "all" ? data.claims.length : data.claims.filter(row => row.classification === item).length}</span></button>)}<div className="method-note"><strong>Fixed controls</strong><p>New York futures trading-day close-date; degraded session-days excluded; 2026-08-11 analysis cutoff.</p></div></aside><div className="claim-list">{claims.map(row => <details key={row.id}><summary><span>{row.id}</span><div><small>{row.classification}</small><strong>{row.claim}</strong></div><b>+</b></summary><div className="claim-detail"><div><span>Source</span><p>{row.source}</p></div><div><span>Sample</span><p>{row.sample}</p></div><div><span>Method</span><p>{row.method}</p></div></div></details>)}</div></div>
    <article className="story-card wide quality-card"><div className="panel-head"><div><p className="eyebrow">Data-source hierarchy</p><h2>NQ first, fallback only when required</h2></div><span className="unit-pill">instrument source</span></div><div className="quality-grid">{fallback.map(row => <div key={row.instrument}><span>Rank {row.rank} · {row.sourceId}</span><strong>{row.instrument}</strong><p>{row.status}</p><small>{row.notes}</small></div>)}</div>{data.meta.fallbackComparability && <p className="chart-caption">{data.meta.fallbackComparability.pair} check: {data.meta.fallbackComparability.commonDaysMinimum}–{data.meta.fallbackComparability.commonDaysMaximum} common eligible days at {data.meta.fallbackComparability.validationHorizonMinutes} minutes. {data.meta.fallbackComparability.status}. {data.meta.fallbackComparability.us100Status}.</p>}<EvidenceNote>Fallbacks are never silently blended. Each result identifies the source instrument and reconciliation status behind its evidence.</EvidenceNote></article>
